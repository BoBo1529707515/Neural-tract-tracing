...
class NeuronTracker:
    ...
    def find_leftward_candidates(self, frame_idx, cx, cy, path, direction, visited, allow_horizontal=True):
        """
        寻找向左的候选点

        关键：dx必须 < 0（严格向左）或 dx==0且dy!=0（垂直移动，可选）
        """
        ...
        for dy in range(-4, 5):
            for dx in range(-4, 5):
                ...
                # dx <= 0 才继续
                # dx < 0: 向左
                # dx == 0: 垂直（仅在allow_horizontal时允许少量）
                if dx == 0 and not allow_horizontal:
                    continue
                ...
                # 方向加成（偏好更向左的）
                left_bonus = abs(dx) / (dist + 0.1)  # dx越负（越向左）越好
                score *= (1 + left_bonus)
    ...
    def get_direction_at_position_no_branch(self, frame_idx, position, max_steps=30):
        """
        在某帧某位置附近获取无分叉的向左方向
        """
        px, py = position
        ...
        if len(path) >= 3:
            ...
            if length > 0 and dx < 0:  # 确保是向左
                return (dx / length, dy / length)

        return (-1, 0)  # 默认向左

    def resolve_branch(self, current_frame, position, candidates, all_paths):
        """遇到分叉时回退到更早帧参考"""
        print(f"    分叉 @ 帧{current_frame}, 位置{position}")
        ...
        for back_frame in range(current_frame - 1, max(-1, current_frame - self.max_backtrack_frames - 1), -1):
            ...
            dir_in_earlier = self.get_direction_at_position_no_branch(back_frame, position)

            if dir_in_earlier is not None and dir_in_earlier[0] < 0:
                # 确保是向左的方向
                ref_direction = dir_in_earlier
    ...
    def trace_leftward_in_frame(self, frame_idx, start_x, start_y, initial_direction, all_paths):
        """
        在单帧内严格向左追踪，直到到达左边界
        """
        frame = self.frames[frame_idx]
        ...
        for step in range(3000):
            ...
            if not candidates:
                ...
                if not candidates:
                    ...
                    if stall_count > 5:
                        # 尝试跳跃：沿当前方向向左跳
                        jump_found = False
                        for jump_dist in range(10, self.max_gap_distance, 5):
                            jx = int(cx + direction[0] * jump_dist)
                            jy = int(cy + direction[1] * jump_dist)
                            ...
                            for dy in range(-8, 9):
                                for dx in range(-8, 1):
                                    nx, ny = jx + dx, jy + dy
                                    if self.is_bright(frame_idx, nx, ny) and (nx, ny) not in visited:
                                        candidates.append((nx, ny, 1.0, nx - cx, ny - cy))
                                        jump_found = True
                                        break
                                if jump_found:
                                    break
                            if jump_found:
                                print(f"    跳跃到 ({jx}, {jy}) 附近")
                                break
            ...
            if is_branch:
                chosen = self.resolve_branch(frame_idx, (cx, cy), sorted_cands, all_paths)
            else:
    ...
    def find_start_in_frame(self, frame_idx, reference_path, direction):
        ...
        if reference_path:
            ...
            for step in range(1, self.max_gap_distance):
                ...
                for dy in range(-8, 9):
                    for dx in range(-8, 1):  # 只在左侧搜
                        if self.is_bright(frame_idx, cx + dx, cy + dy):
                            return (cx + dx, cy + dy)

        return None

    def trace_backward_strict_left(self, start_frame_idx, start_x, start_y):
        """
        主函数：严格向左追踪到最左边界
        """
        print(f"\n{'=' * 60}")
        ...
        print(f"目标: 到达左边界 (x <= {self.left_margin})")
        print(f"{'=' * 60}")
        ...
        # 只向左追踪
        current_path = self.trace_leftward_in_frame(
        ...
        for frame_idx in range(start_frame_idx - 1, -1, -1):
            ...
            if start_pt:
                new_path = self.trace_leftward_in_frame(
                    frame_idx, start_pt[0], start_pt[1], direction, all_paths
                )
...
class TrackerGUI:
    ...
    def setup_controls(self, parent):
        ...
        ttk.Label(g, text="最大回退:").grid(row=1, column=2, sticky="w", padx=(8, 0))
        self.backtrack_var = tk.StringVar(value="10")
        ttk.Entry(g, textvariable=self.backtrack_var, width=5).grid(row=1, column=3)

        ttk.Button(f4, text="应用参数", command=self.apply_params).pack(pady=3)

        # 算法
        f5 = ttk.LabelFrame(parent, text="📐 严格向左算法", padding=5)
...
神经元发育是单向的，不能向右！"""
    ...
    def load_frames(self):
        ...
        if self.tracker.load_frames(self.input_dir, lambda p: (
            ...
            self.status.set(f"已加载 {n} 帧 ({self.tracker.width}x{self.tracker.height})")
        else:
    ...
    def on_click(self, e):
        ...
        paths = self.tracker.trace_backward_strict_left(self.current_frame_idx, ix, iy)
        ...
        self.preview_info.set(f"F{f_min}→F{f_max}, 左x={leftmost} {reached}")
    ...
    def update_result_list(self):
        ...
        for i, (paths, _, _) in enumerate(self.tracker.tracking_results):
            f_min, f_max = min(paths.keys()), max(paths.keys())
            leftmost = min(p[0][0] for p in paths.values() if p)
            self.result_list.insert(tk.END, f"#{i + 1}: F{f_min}-{f_max}, 左x={leftmost}")
    ...
    def export_video(self):
        ...
        path = os.path.join(self.output_dir, "tracking_strict_left.mp4")
    ...
    def render(self, idx, for_export=False):
        ...
        for i, (paths, _, _) in enumerate(self.tracker.tracking_results):
            ...
            if idx in paths:
                ...
                if path:
                    cv2.circle(vis, path[0], 5, (0, 255, 0), -1)  # 左端绿
