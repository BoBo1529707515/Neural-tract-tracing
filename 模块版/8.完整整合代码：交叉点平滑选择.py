...
class NeuronTracker:
    ...
    def __init__(self):
        ...
        # 交叉点检测参数
        self.intersection_score_ratio = 0.7  # 高分候选的比例阈值
        self.intersection_angle_threshold = 0.87  # cos(30°)
    ...
    def trace_single_frame(self, frame_idx, start_x, start_y, initial_direction=None):
        ...
        return path, boundary_reached, intersection_count
    ...
    def trace_with_multi_frame_overlay(self, start_frame_idx, start_x, start_y):
        ...
        for frame_idx in range(earliest_frame, latest_frame + 1):
            if self.has_signal_near(frame_idx, start_x, start_y):
                ...
                if best_pt:
                    path, boundary, intersections = self.trace_single_frame(frame_idx, best_pt[0], best_pt[1])
                    ...
                    if len(path) > 5:
                        ...
                        inter_str = f"(交叉:{intersections})" if intersections > 0 else ""
                        print(f"  帧 {frame_idx}: {len(path)}点, 左x={left_x} {boundary_str} {inter_str}")
        ...
        print(f"\n边界统计: 左={boundary_counts['left']}, 上={boundary_counts['top']}, 下={boundary_counts['bottom']}")
        print(f"交叉点总数: {total_intersections}")
        ...
        return all_paths, paths_by_frame, final_path, (earliest_frame, latest_frame), (final_boundary_reached,
                                                                                       final_boundary_type), total_intersections
...
class TrackerGUI:
    ...
    def __init__(self):
        ...
        self.preview_intersection_count = 0
    ...
    def setup_controls(self):
        ...
        ttk.Label(g, text="交叉阈值:").grid(row=3, column=0, sticky="w", pady=2)
        self.inter_ratio_var = tk.StringVar(value="0.7")
        ttk.Entry(g, textvariable=self.inter_ratio_var, width=5).grid(row=3, column=1)

        ttk.Button(f4, text="应用参数", command=self.apply_params).pack(pady=3)

        # 算法说明
        f5 = ttk.LabelFrame(p, text="📐 交叉点处理", padding=5)
    ...
    def on_click(self, e):
        ...
        if result:
            all_paths, individual_paths, final_path, signal_range, boundary_info, inter_count = result
            ...
            self.preview_intersection_count = inter_count
            ...
            self.preview_info.set(
                ...
                f"交叉点: {inter_count} | {boundary_str}"
            )
    ...
    def confirm(self):
        ...
        self.tracker.tracking_results.append((
            ...
            self.preview_intersection_count
        ))
        self.preview_paths = {}
        self.preview_individual_paths = {}
        self.preview_final_path = []
    ...
    def apply_params(self):
        try:
            ...
            self.tracker.intersection_score_ratio = float(self.inter_ratio_var.get())
        except:
            pass
    ...
    def update_result_list(self):
        ...
        for i, item in enumerate(self.tracker.tracking_results):
            paths, final_path, _, signal_range, boundary_info, inter_count = item
            ...
            self.result_list.insert(tk.END, f"#{i + 1}: 帧{e}~{l} {bstr} 交叉:{inter_count}")
    ...
    def export_video(self):
        ...
        path = os.path.join(self.output_dir, "tracking_smooth.mp4")
