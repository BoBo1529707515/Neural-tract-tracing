...
class NeuronTracker:
    ...
    def __init__(self):
        ...
        self.max_gap_distance = 50

        # 分叉处理
        self.branch_score_ratio = 0.85
        self.max_backtrack_frames = 10  # 最多回退多少帧
    ...
    def detect_branch_at_position(self, frame_idx, position, path, direction):
        """
        检测在指定帧的指定位置是否存在分叉

        返回: (is_branch, candidates, best_direction)
        """
        cx, cy = position
        ...
        if len(candidates) < 2:
            ...
            if candidates:
                ...
                dist = np.sqrt(dx ** 2 + dy ** 2)
                best_dir = (dx / dist, dy / dist) if dist > 0 else None
            return False, candidates, best_dir

        # 检测分叉
        sorted_cands = sorted(candidates, key=lambda x: -x[2])
        best_score = sorted_cands[0][2]

        high_score_cands = [c for c in sorted_cands if c[2] >= best_score * self.branch_score_ratio]
        ...
        if len(high_score_cands) >= 2:
            ...
            if len(dirs) >= 2:
                ...
                if cos_angle < 0.5:  # 夹角>60度
                    # 是分叉
                    dx, dy = sorted_cands[0][3], sorted_cands[0][4]
                    dist = np.sqrt(dx ** 2 + dy ** 2)
                    return True, sorted_cands, (dx / dist, dy / dist) if dist > 0 else (-1, 0)

        # 不是分叉
        dx, dy = sorted_cands[0][3], sorted_cands[0][4]
        dist = np.sqrt(dx ** 2 + dy ** 2)
        return False, sorted_cands, (dx / dist, dy / dist) if dist > 0 else (-1, 0)

    def get_direction_at_position_in_frame(self, frame_idx, position, search_range=20):
        """
        获取某帧中某位置附近的轨迹方向（不分叉时的方向）
        ...
        for _ in range(30):  # 最多追踪30步
            # 检测是否有分叉
            is_branch, candidates, next_dir = self.detect_branch_at_position(
                frame_idx, (cx, cy), path, direction
            )
    ...
    def resolve_branch_by_earlier_frames(self, current_frame_idx, position, candidates, paths):
        """
        遇到分叉时，回退到更早的帧（N-1, N-2...）找没有分叉的方向

        current_frame_idx: 当前帧号
        position: 分叉位置
        candidates: 分叉候选
        paths: 已经处理过的帧的路径 {frame_idx: path}

        返回: 选择的候选 (nx, ny, ...)
        """
        print(f"    分叉检测 @ 帧{current_frame_idx}, 位置{position}")
        print(f"    候选数: {len(candidates)}")

        # 从 N-1 帧开始回退
        reference_direction = None

        for back_frame in range(current_frame_idx - 1, max(-1, current_frame_idx - self.max_backtrack_frames - 1), -1):
            if back_frame < 0:
                break

            # 在更早的帧中获取该位置附近的方向
            dir_in_earlier_frame = self.get_direction_at_position_in_frame(back_frame, position)

            if dir_in_earlier_frame is not None:
                # 检查这个帧在这个位置是否也有分叉
                is_branch_earlier, _, _ = self.detect_branch_at_position(
                    back_frame, position, [], dir_in_earlier_frame
                )

                if not is_branch_earlier:
                    # 找到了没有分叉的帧！
                    reference_direction = dir_in_earlier_frame
                    print(f"    回退到帧{back_frame}找到无分叉方向: {reference_direction}")
                    break
                else:
                    print(f"    帧{back_frame}也有分叉，继续回退...")

        if reference_direction is None:
            # 所有回退的帧都有分叉，使用默认向左
            reference_direction = (-1, 0)
            print(f"    所有回退帧都有分叉，使用默认向左")
        ...
        for c in candidates:
            ...
            dist = np.sqrt(dx ** 2 + dy ** 2)
            if dist > 0:
                cos_a = (dx * reference_direction[0] + dy * reference_direction[1]) / dist
                if cos_a > best_dir_score:
                    best_dir_score = cos_a
                    best_candidate = c

        if best_candidate:
            print(f"    选择方向与参考一致的候选: ({best_candidate[0]}, {best_candidate[1]})")
            return best_candidate

        return candidates[0] if candidates else None

    def trace_in_frame_with_branch_handling(self, frame_idx, start_x, start_y, initial_direction, all_paths):
        """
        在单帧内追踪，遇到分叉时回退到更早的帧参考

        all_paths: 所有已处理帧的路径（用于分叉时参考）
        """
        frame = self.frames[frame_idx]
        ...
        for step in range(2000):
            ...
            # 检测分叉
            is_branch, candidates, default_dir = self.detect_branch_at_position(
                frame_idx, (cx, cy), path, direction
            )
                ...
                for r in range(4, 8):
                    for dy in range(-r, r + 1):
                        for dx in range(-r, r + 1):
                            ...
                            if self.is_bright(frame_idx, nx, ny):
                                dist = np.sqrt(dx ** 2 + dy ** 2)
                                candidates.append((nx, ny, 1.0, dx, dy))
                    if candidates:
                        break
            ...
            # 处理分叉
            if is_branch:
                chosen = self.resolve_branch_by_earlier_frames(
                    frame_idx, (cx, cy), candidates, all_paths
                )
            else:
            ...
            if dist > 0:
                ...
                if direction:
                    ...
                    dlen = np.sqrt(direction[0] ** 2 + direction[1] ** 2)
                    if dlen > 0:
    ...
    def trace_backward_with_branch_backtrack(self, start_frame_idx, start_x, start_y):
        """
        主追踪函数：向前回溯，分叉时回退到更早帧参考

        处理顺序：从 start_frame_idx 向 0 回溯
        分叉处理：在帧N遇到分叉时，查看帧N-1、N-2...直到无分叉
        """
        ...
        # 双向追踪
        print(f"\n处理帧 {start_frame_idx} (起始帧)...")

        left_path = self.trace_in_frame_with_branch_handling(
            start_frame_idx, start_x, start_y, (-1, 0), all_paths
        )

        right_path = self.trace_in_frame_with_branch_handling(
            start_frame_idx, start_x, start_y, (1, 0), all_paths
        )

        # 合并
        right_rev = right_path[::-1]
        if len(right_rev) > 1:
            current_path = right_rev[:-1] + left_path
        else:
        ...
        print(f"  路径: {len(current_path)} 点")
        if current_path:
            print(f"  范围: x={current_path[0][0]} ~ {current_path[-1][0]}")
        ...
        if len(prev_path) >= 2:
            ...
            length = np.sqrt(dx ** 2 + dy ** 2)
            backward_dir = (dx / length, dy / length) if length > 0 else (-1, 0)
        else:
            backward_dir = (-1, 0)

        # 确保向左
        if backward_dir[0] > 0:
            backward_dir = (-backward_dir[0], -backward_dir[1])
        ...
        for frame_idx in range(start_frame_idx - 1, -1, -1):
            ...
            # 找起点
            start_pt = self.find_start_in_frame(frame_idx, prev_path, backward_dir)

            if start_pt:
                # 追踪
                new_path = self.trace_in_frame_with_branch_handling(
                    frame_idx, start_pt[0], start_pt[1], backward_dir, all_paths
                )

                # 双向
                opposite_dir = (-backward_dir[0], -backward_dir[1])
                other_path = self.trace_in_frame_with_branch_handling(
                    frame_idx, start_pt[0], start_pt[1], opposite_dir, all_paths
                )

                # 合并
                other_rev = other_path[::-1]
                if len(other_rev) > 1:
                    combined = other_rev[:-1] + new_path
                else:
                    combined = new_path

                # 去重
                seen = set()
                combined = [p for p in combined if not (p in seen or seen.add(p))]

                # 排序
                if len(combined) >= 2 and combined[0][0] > combined[-1][0]:
                    combined = combined[::-1]

                all_paths[frame_idx] = combined
                prev_path = combined

                # 更新方向
                if len(combined) >= 2:
                    dx = combined[0][0] - combined[-1][0]
                    dy = combined[0][1] - combined[-1][1]
                    length = np.sqrt(dx ** 2 + dy ** 2)
                    if length > 0:
                        backward_dir = (dx / length, dy / length)
                        if backward_dir[0] > 0:
                            backward_dir = (-backward_dir[0], -backward_dir[1])
                ...
                if frame_idx % 20 == 0 or frame_idx < 5:
                    left_x = combined[0][0] if combined else -1
                    print(f"  帧 {frame_idx}: {len(combined)}点, 左端x={left_x}")
...
class TrackerGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("神经元追踪 - 分叉回退到更早帧")
    ...
    def setup_controls(self, parent):
        ...
        ttk.Label(g, text="分叉阈值:").grid(row=1, column=2, sticky="w", padx=(8, 0))
        self.branch_var = tk.StringVar(value="0.85")
        ttk.Entry(g, textvariable=self.branch_var, width=5).grid(row=1, column=3)

        ttk.Label(g, text="最大回退:").grid(row=2, column=0, sticky="w", pady=2)
        self.backtrack_var = tk.StringVar(value="10")
        ttk.Entry(g, textvariable=self.backtrack_var, width=5).grid(row=2, column=1)

        ttk.Button(f4, text="应用参数", command=self.apply_params).pack(pady=3)

        # 算法
        f5 = ttk.LabelFrame(parent, text="📐 分叉回退算法", padding=5)
...
5. 用该帧方向决定选择"""
        ...
        ttk.Label(f6, text="左键: 选点 | 右键: 取消 | Enter: 确认").pack(anchor="w")
        ...
        # 结果
        f7 = ttk.LabelFrame(parent, text="📊 结果", padding=5)
    ...
    def on_click(self, e):
        ...
        paths = self.tracker.trace_backward_with_branch_backtrack(self.current_frame_idx, ix, iy)
        ...
        self.preview_info.set(f"F{f_min}→F{f_max}, {total_pts}点, 左x={leftmost}")
    ...
    def apply_params(self):
        try:
            ...
            self.tracker.branch_score_ratio = float(self.branch_var.get())
    ...
    def update_result_list(self):
        ...
        for i, (paths, _, _) in enumerate(self.tracker.tracking_results):
            ...
            leftmost = min(p[0][0] for p in paths.values() if p)
            self.result_list.insert(tk.END, f"#{i + 1}: F{f_min}-F{f_max}, 左x={leftmost}")
    ...
    def export_video(self):
        ...
        path = os.path.join(self.output_dir, "tracking.mp4")
    ...
    def render(self, idx, for_export=False):
        ...
        cv2.line(vis, (self.tracker.left_margin, 0),
                 (self.tracker.left_margin, self.tracker.height), (50, 50, 50), 1)
