    ...
    def trace_with_multi_frame_overlay(self, start_frame_idx, start_x, start_y):
        ...
        for frame_idx in range(earliest_frame, latest_frame + 1):
            if self.has_signal_near(frame_idx, start_x, start_y):
                ...
                if best_pt:
                    path = self.trace_single_frame(frame_idx, best_pt[0], best_pt[1])
                    if len(path) > 5:
                        paths_by_frame[frame_idx] = path
                        left_x = min(p[0] for p in path)
                        print(f"  帧 {frame_idx}: {len(path)}点, 左端x={left_x}")
        ...
        print(f"  最终路径: {len(final_path)}点, 最左x={left_x}")
        ...
        return all_paths, paths_by_frame, final_path, (earliest_frame, latest_frame)
...
class TrackerGUI:
    ...
    def setup_controls(self):
        ...
        # 显示选项
        f5 = ttk.LabelFrame(p, text="👁️ 显示", padding=5)
        f5.pack(fill="x", pady=pad, padx=3)
        self.show_individual_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(f5, text="显示各帧独立轨迹", variable=self.show_individual_var,
        ...
        ttk.Label(f6, textvariable=self.preview_info, foreground="blue", wraplength=300).pack(anchor="w", pady=3)
        ...
        ttk.Label(f9, textvariable=self.status, wraplength=300, foreground="blue").pack(fill="x")
    ...
    def on_click(self, e):
        ...
        if result:
            all_paths, individual_paths, final_path, signal_range = result
            ...
            self.preview_info.set(
                f"信号帧: {earliest}~{latest} ({n_frames}帧参与)\n"
                f"叠加路径: {len(final_path)}点, 最左x={leftmost}"
            )
    ...
    def update_result_list(self):
        ...
        for i, (paths, final_path, _, signal_range) in enumerate(self.tracker.tracking_results):
            leftmost = final_path[0][0] if final_path else -1
            e, l = signal_range if signal_range else (0, 0)
            self.result_list.insert(tk.END, f"#{i + 1}: 帧{e}~{l}, 左x={leftmost}")
    ...
    def export_video(self):
        ...
        path = os.path.join(self.output_dir, "tracking_overlay.mp4")
    ...
    def render(self, idx, for_export=False):
        ...
        for i, (paths, final_path, _, _) in enumerate(self.tracker.tracking_results):
