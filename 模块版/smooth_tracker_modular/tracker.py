...
class NeuronTracker:
    ...
    def local_recompute_with_waypoints(self, neuron_id, new_waypoints,
        ...
        """
        局部重算逻辑：
        - 新点 X 在已有 waypoints 之间 → 只清除夹它的两锚点之间的路径段，
          重追该段（经过段内所有旧 waypoints + 新点），左右两侧严格不动
        - 新点 X 小于最左 waypoint    → 清除路径左端，从新点重追
        - 新点 X 大于最右 waypoint    → 清除路径右端，从最右锚点追到新点，
          锚点左侧严格不动
        全程遵守白线 + 平滑（转角）约束。
        """
        ...
        def build_segment(start_pt, targets, end_pt):
            ...
            依次经过 targets，最终到达 end_pt。
            """
            seg = [start_pt]
            cur = start_pt

            # 从 base_path 中估算 start_pt 附近的前向方向
            ...
            # trace_segment_left 是向左追踪器；取反方向来实现向右追踪
        ...
        for new_pt in new_waypoints:
            ...
            # ── 情况 2：新点在最右端之外 ──────────────────────────────
                ...
                # 强制首点为 anchor_point，防止段污染 before
        ...
        for frame_idx in range(first_marked, len(self.frames)):
            new_pts, right_boundary, new_direction = self.grow_rightward(
                frame_idx, current_path, current_direction
            )
            if new_pts:
                current_path      = current_path + new_pts
                current_direction = new_direction
            paths_by_frame[frame_idx] = current_path.copy()
        ...
        print(f"局部重算完成，initial_path={len(initial_path)}点，"
              f"最终={len(max_path)}点，左端是否变动={left_modified}")
        return new_result
    ...
    def compute_all_speeds(self, fps=10.0, pixel_um=1.0):
        """批量计算所有已追踪神经元的速度 {neuron_id: [speed_records]}"""
        return {
