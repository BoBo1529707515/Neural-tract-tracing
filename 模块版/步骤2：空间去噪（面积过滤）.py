...
# ╔══════════════════════════════════════════════════════════════╗
# ║  步骤2: 空间去噪 · 去除小面积噪点 (Area>=20)                   ║
# ╚══════════════════════════════════════════════════════════════╝

# === 输入输出路径 ===
INPUT_DIR = r"F:\工作文件\RA\python\项目汇总\神经图像\01_二值化基础\frames_binary"
OUTPUT_DIR = r"F:\工作文件\RA\python\项目汇总\神经图像\02_空间去噪"

# === 🎛️ 去噪参数 ===
MIN_AREA = 30  # 最小保留面积（像素数）
...
def main():
    print("╔" + "═" * 58 + "╗")
    print("║" + "  步骤2: 空间去噪 · 面积过滤 (Area>=20)".center(48) + "║")
    print("╚" + "═" * 58 + "╝")
    ...
    if not os.path.exists(INPUT_DIR):
        print(f"❌ 目录不存在! 请先运行步骤1（CLAHE二值化）")
        return
    ...
    # === 创建输出目录 ===
    frames_denoised_dir = os.path.join(OUTPUT_DIR, "frames_denoised")
    os.makedirs(frames_denoised_dir, exist_ok=True)
    ...
    # 去噪后视频
    video_denoised_path = os.path.join(OUTPUT_DIR, "denoised_all_frames.mp4")
    ...
    for idx, filename in enumerate(frame_files):
        ...
        save_image(os.path.join(frames_denoised_dir, output_filename), denoised)

        # 写入去噪视频
        denoised_resized = cv2.resize(denoised, (video_w, video_h))
        out_denoised.write(denoised_resized)

        # 写入对比视频
        small_w = compare_w // 2

        orig_small = cv2.resize(binary, (small_w, compare_h))
        denoised_small = cv2.resize(denoised, (small_w, compare_h))

        # 添加标签
        cv2.putText(orig_small, f"Before #{idx} ({white_before // 1000}k)", (5, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, 255, 2)
        cv2.putText(denoised_small, f"After ({white_after // 1000}k) -{removed}obj", (5, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, 255, 2)

        compare_frame = np.hstack([orig_small, denoised_small])
    ...
    # === 保存统计数据 ===
    stats_path = os.path.join(OUTPUT_DIR, "denoise_statistics.csv")
    ...
    print(f"\n   对象统计:")
    print(f"     总共去除:   {total_removed:,} 个噪点")
    ...
    # === 保存参数 ===
    params_path = os.path.join(OUTPUT_DIR, "denoise_params.txt")
    ...
    {OUTPUT_DIR}/
    │
    ├── frames_denoised/         # 去噪后的二值图 (后续处理用)
    │   ├── frame_0000.png
    │   ├── frame_0001.png
    │   └── ... ({num_frames} 张)
    │
    ├── denoised_all_frames.mp4  # 去噪后视频
    ├── compare_before_after.mp4 # 对比视频
    ├── denoise_statistics.csv   # 统计数据
    └── denoise_params.txt       # 参数记录
    """)

    print("✅ 步骤2完成！后续对齐裁剪使用 frames_denoised/ 中的文件。")
