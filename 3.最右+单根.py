...
class NeuronTrackerSinglePath:
    ...
    def trace_single_path(self, skeleton, start_point):
        """
        从起始点追踪单条路径（不分叉）
        遇到分叉时选择最"向左且水平"的方向
        """
        ...
        while True:
            ...
            # 选择最佳邻居：优先向左(dx<0)且水平(|dy|小)
            def score_neighbor(n):
                ny, nx, dy, dx = n
                # 向左加分，水平加分
                left_score = -dx * 10  # dx=-1 得+10分
                horizontal_score = -abs(dy) * 5  # dy=0 得0分，dy=±1 得-5分
                return left_score + horizontal_score
    ...
    def draw_trajectory(self, image, trajectory, color=(0, 255, 255), thickness=2):
        ...
        info = f"Neuron | Length: {len(smoothed)} px | X-span: {x_span:.0f} px"
    ...
    def process_video(self, video_path, output_path, show_preview=True):
        ...
        while True:
            ...
            rightmost = self.find_rightmost_point(skeleton)
            trajectory = self.trace_single_path(skeleton, rightmost)
            ...
            if can_show:
                ...
                cv2.imshow('Single Path Neuron (Q to quit)', display)
